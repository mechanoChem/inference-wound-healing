...
for gg in range(nGroups):
  ...
  for ss in range(1):
    ...
    gamma_matrix = np.loadtxt(adjSaveDirectory+adjSaveName)
    print('~~~~~~~~~~~')
  ...
  #Shouldn't change
    ...
    ################# Define residue (without Boundary condition) #################
      ...
      for step in range(total_num_step-1):
        ...
        Error_data_n.vector()[:] = 100*np.abs( (C.vector()[:]-C_data_n.vector()[:]) /(0+C_data_n.vector()[:]))
