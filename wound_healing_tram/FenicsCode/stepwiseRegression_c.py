...
def fit_lsq_linear(theta_matrix,X_matrix,bounds,lsmr_tol='auto', verbose=0 ):
  res=lsq_linear(theta_matrix, X_matrix,bounds=bounds, lsmr_tol='auto', verbose=0)
  gamma_vector=res.x
  loss=np.mean(np.square(np.matmul(theta_matrix,gamma_vector)-X_matrix))
...
class stepwiseR(object):
  def __init__(self,F_criteria=[1],F_switch=[],basis_drop_strategy='aggressive',sigma_n=1.0e-8,anchor_index=[-1],group=[-1],alpha_lasso=0,alpha_ridge=0, ridge_cv=[-1],threshold_d=1.0e-14,n_jobs=1):
      ...
      for j in range(gamma_vector.size):
        ...
        if abs(gamma_vector[j])<gamma_criteria :
          ...
          if F<F_threshold or loss_try<self.sigma_n:
            ...
            if self.group[0]!=-1:
              for group_index in range(len(self.group)):
                if local_to_global_index[j] in self.group[group_index]:
                  self.group[group_index].remove(local_to_global_index[j])
