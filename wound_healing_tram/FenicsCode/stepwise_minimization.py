...
def stepwise_minimization(obj_f, x0, args_dict,F_threshold=1.0e16, method_options={}, grad_f=None, save_to_file=None):
  ##
  callback=None
  bounds=None
  current_activate_index=args_dict['activate_basis_index']
  target_index=args_dict['target_vector_index']
  used_index=args_dict['used_data_index']
  ...
  res=sp.minimize(obj_f, x0,jac=grad_f, method=method, args=(current_activate_index,target_index, used_index),bounds=bounds, options=method_options,callback=callback )
  x0=res.x
  gamma_matrix[current_activate_index,0]=x0
  loss[0]=res.fun
  print('==============================================================')
  print('step=',0, ' current_activate_index=',current_activate_index,' x0=',gamma_matrix[:,0],' loss=',loss[0] )
  if save_to_file!=None:
  ...
  for step in range(len(current_activate_index)-1):
    ...
    loss_tem=np.ones(num_activate_index)*1.0e20
    for j in range(len(current_activate_index)):
      ...
      res=sp.minimize(obj_f, x0_tem,jac=grad_f,  method=method, args=(current_activate_index_tem,target_index, used_index),bounds=bounds_tem, options=method_options,callback=callback)
