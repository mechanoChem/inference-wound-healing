...
runtype = 1 #1 for VSI, 2 for Adjoint, anything else for user supplied (see near line 175 and change variable: gamma_list_userSupplied)
oneD_flag = True
...
if runtype ==1:
    ...
    if oneD_flag:
        if temporalParam_flag:
            ...
            print(f'Running VSI iter {vsiIter}')
    else:
        if temporalParam_flag:
    ...
    if initCondDensity == 10000:
      gamma_list_userSupplied = [310.,0.,0.,0.,0.,0.,0.044, -25.8823529411765]
    elif initCondDensity == 12000:
      gamma_list_userSupplied = [250.,0.,0.,0.,0.,0.,0.044, -25.8823529411765]
    elif initCondDensity == 14000:
      gamma_list_userSupplied = [720.,0.,0.,0.,0.,0.,0.048, -28.2352941176471]
    elif initCondDensity == 16000:
      gamma_list_userSupplied = [570.,0.,0.,0.,0.,0.,0.049, -28.8235294117647]
    elif initCondDensity == 18000:
      gamma_list_userSupplied = [760.,0.,0.,0.,0.,0.,0.054, -31.7647058823529]
    elif initCondDensity == 20000:
      gamma_list_userSupplied = [1030.,0.,0.,0.,0.,0.,0.064, -37.6470588235294]
  #  gamma_list_userSupplied = [0.00000000e+00, 0.00000000e+00, 1.3806921091993574, 0.00000000e+00, 0.00000000e+00, 0.00000000e+00,  3.624326447274345e-12, 0.00000000e+00, 0.00000000e+00, 0.00000000e+00, 9.465794550752025e-06, 0.00000000e+00, 0.00000000e+00]
   #m =  [ 4.47313227 -0.05811537  0.26190422]   
    gamma_matrix = np.empty((total_num_step,),dtype=object) 
...
directory = '../results/forward_solution/'+plot_tag+'/initCond' + str(initCondDensity) + '/step' + str(vsiIter)
...
for step in range(total_num_step):
  ...
  #Error
  Error_data_n.vector()[:] = 100*np.abs( (C.vector()[:]-C_data_n.vector()[:]) /(1e-15+C_data_n.vector()[:]))
